...
namespace ApiEcommerce.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public IActionResult SearchProducts(string searchTerm)
        {
            ...
            if (products.Count == 0)
            {
                return NotFound($"Los productos con los nombres o descripcion '{searchTerm}' no existen");
            }
            var productsDto = _mapper.Map<List<ProductDto>>(products);
        ...
        [HttpPatch("buyProduct/{name}/{quantity}", Name = "BuyProduct")]
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public IActionResult BuyProduct(string name, int quantity)
        {
            if (string.IsNullOrWhiteSpace(name) || quantity <= 0) {
                return BadRequest("El nombre de producto o la cantidad no son validos");
            }

            var foundProduct = _productRepository.ProductExists(name);
            if (!foundProduct)
            {
                return NotFound($"El mproducto con el nombre {name} no existe");
            }
            ...
            return Ok($"Se compro la cantidad {quantity} {units} del producto '{name}'");
